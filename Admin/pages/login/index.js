    ...
    <div className={styles["login-container"]}>
      <div className={styles["image-container"]}>
        <div className={styles["text-wrapper"]}>
          ...
          <p style={{ marginTop: "10px" }}>Admin's Page</p>
        </div>
        <img
      ...
      <div className={styles["form-panel"]}>
        ...
        <form onSubmit={loginFormHandler} className={styles["form-container"]}>
          ...
          {!error && progress && (
            <CircularProgress size={70} style={{ marginLeft: "100px" }} />
          )}
...
export async function getServerSideProps() {
  ...
  const [adminsDB] = await Promise.all([
    fetch(connection_string + "admins", headers),
  ]);

  const adminsCollection = await adminsDB.json();
  const admins = adminsCollection.admins;
